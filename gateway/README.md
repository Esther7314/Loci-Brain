...
1. **它太宽** —— 日常说话几乎每句都过，于是每一轮都跳一行「有 N 条相关记忆」。
   **每轮都响的提醒等于没有提醒。**
2. **每次触发都要等一次搜索，而那个时间跟你的库有多大走。**
   我们 1300 条的库上约 3 秒；你要是刚导进来两万条历史，那就是每一轮都等这么久。
   👉 **开之前先自己跑一次 `recall(query="随便什么")` 掐个表** ——
   这个数只有你自己能量，我们的数说明不了你的情况。

你更怕漏掉、不怕吵，而且掐表掐出来的数你能接受，就 `RELEVANCE_WEAK=1` 打开。
...
🔴 **12 秒这个默认值是照我们 1300 条的库定的，不是普适的。**
