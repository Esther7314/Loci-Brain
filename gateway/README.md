...
- **强档** = 这句话里出现了明说要翻旧账的词（`上次` / `之前` / `还记得` …）。
  仓库带的是中文表：`RELEVANCE_STRONG_WORDS="remember,last time,earlier"` 整表换掉，
  不分大小写。**不换的话，不说中文的人一次都不会触发。**
- **弱档** = 过滤掉应声词之后但凡有实质内容就放行。**默认关**（`RELEVANCE_WEAK=1` 打开）。

  关着的理由**只有一条：它太宽** —— 日常说话几乎每句都过，于是「有相关记忆」
  变成每轮都在响，而每轮都响的提醒等于没有提醒。
  （不是因为慢。一次带 `query` 的 `recall` 在 1300 条的库上约 3 秒，开着也扛得住。）

  **什么时候值得开**：你更怕漏掉，而不是更怕吵。开之前先想清楚哪种更难受。

⚠️ **超时给太少 = 这功能悄悄不工作。** 它原来写死 5 秒，而带 `query` 的 `recall`
要好几秒（1300 条的库实测约 3 秒：向量 + BM25 跑一遍全库，**跟库大小走**）。
于是它每次都超时中止、命中数恒为 0，**从上线到发现一次都没成功过**；
而失败只进日志、聊天照常 —— **界面上永远是正常的**。
判断它有没有在工作：看日志里 `relevance_reminder_observed` 的 `injected` 和 `error`。

📌 **这个数要跟着你的库长。** 装完先自己跑一次 `recall(query="随便什么")` 掐个表，
把 `RELEVANCE_TIMEOUT_MS` 设成它的两倍。
