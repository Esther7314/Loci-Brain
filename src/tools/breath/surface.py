...
from locibrain.policy.surfacing import SurfacePolicyVM
from .. import _runtime as rt
from utils import is_closed, parse_bool, parse_iso_datetime
...
async def surface_default(max_results: int, max_tokens: int, tag_filter: list) -> str:
    ...
    unresolved = [
        ...
        if _can_surface(b)
        and not is_closed(b["metadata"])  # 终点只认 status；旧布尔只读兼容（二改第0节）
    ...
    if not primary_omitted and random.random() < 0.03:
        try:
            ...
            resolved_pool = [
                b for b in all_buckets
                if _can_surface(b)
                and is_closed(b["metadata"])  # 终点只认 status；旧布尔只读兼容（二改第0节）
