...
from locibrain.app.profiles import build_default_legacy_profiles
from locibrain.app.legacy_runtime import LegacyRuntime
from locibrain.architecture import (
    ADRDocument,
...
from locibrain.cluster.replication import ReplicationContract, ReplicationSegment, ReplicationTopology
from locibrain.domain import AdvancedCommandBoundaryContract, BoundaryStage, CommandBoundaryReceipt
from locibrain.domain.commands import CommandKind, MemoryCommand, MemoryCommandRouter
from locibrain.kernel.errors import PolicyViolation
from locibrain.observability import ObservabilityMetricBoundary
from locibrain.maintenance.migration_contract import (
    MigrationPhasePlan,
    MigrationPreservationContract,
    MigrationTraceRecord,
)
from locibrain.maintenance.vnext_coverage import VNextCoverageMatrix
from locibrain.plugins import PluginManifest, PluginRuntime, PluginSandbox
from locibrain.policy import RedLineContract, RedLineFeatureSpec, SurfaceDecision
from locibrain.policy.engine import PolicyEngine
from locibrain.policy.formal_invariants import FormalInvariantChecker
from locibrain.protocol import PublicToolDesignContract, PublicToolSpec, ToolExposure
from locibrain.resilience import CrashRecoveryContract, CrashRecoveryPlan, PathStep
from locibrain.retrieval import (
    MemoryContextCompiler,
    RetrievalCandidate,
    RetrievalFeatures,
)
from locibrain.resilience.scanner import V3ResilienceScanner
from locibrain.eventsourcing.ledger_mirror import LedgerMirror
from locibrain.eventsourcing.ledger_property import LedgerReplayPropertyRunner
...
def _rust_kernel_scaffold_check() -> dict[str, Any]:
    ...
    crate = repo_root / "kernel" / "rust" / "loci-kernel"
    ...
    manifest_snippets = ('name = "loci-kernel"', "[dependencies]")
    lib_snippets = (
...
def _code_standards_check() -> dict[str, Any]:
    report = HighestDifficultyCodeStandards.default().evaluate_manifest(
        [
            ...
            CodeArtifactSpec(
                path="src/locibrain/policy/surfacing.py",
                language=ArtifactLanguage.PYTHON,
